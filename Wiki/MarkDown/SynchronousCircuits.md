...
Some examples of clock definitions are presented below:
    ...
    clk<frequency = "133 MHz", tolerance = "-5, 0", type = "PLL"     > DDRClock(BaseClock);
    clk<frequency =  "10 MHz",                      type = "PLL"     > CPUClock(BaseClock);
    clk<frequency =   "1 Hz" ,                      type = "Enable"  > RTC     (CPUClock );
    clk<frequency =   "5 MHz",                      type = "Enable"  > UserClk (CPUClock );
    clk<frequency =  "10 MHz",                      type = "Internal"> CPUGated(CPUClock );
...
All clocks (including clock pins) have a `frequency` member (of type `rat`) that can be used by scripts to calculate clock-frequency dependent parameters.  A button denouncer might, for instance, be defined as follows:
    ...
    :::C++
    class Debouncer(clk Clk, net Input){
     out net Output;
